RSpec.describe 'inject pattern test' do
  ...
  it 'test 6' do
    ...
    toppings = []
    topping_calories.each do |name, _|
      toppings << name.to_s
    end
  ...
  it 'test 10' do
    ...
    menu.each do |item, details|
      ...
      if details[:gluten_free]
        gluten_message = "(gluten free)"
      else
        gluten_message = "(non gluten free)"
      end

      last_flavor = details[:flavors].last
      details[:flavors][-1] = "and #{last_flavor}"
      flavors = details[:flavors].join(", ")

      printable_menu += "- #{flavors} #{item} #{gluten_message}\n"
    end
